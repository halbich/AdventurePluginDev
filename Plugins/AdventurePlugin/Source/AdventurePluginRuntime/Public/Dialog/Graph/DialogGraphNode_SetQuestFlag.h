...
public:
	...
	*/
	...
	virtual inline FText GetNodeTitle() const
	{
		return FText::Format(NSLOCTEXT("DialogGraphNode_SetQuestFlag", "NodeTitle", "SET {0}->{1}"), 
			FText::FromString(IsValid(Flag.Quest) ? Flag.Quest->Name : TEXT("<EMPTY>")),
	...
	*/
	virtual bool Execute(UAdventurePluginGameContext* GameContext) override
	{
		...
		{
			LOG_Error(NSLOCTEXT("AdventurePlugin", "DialogGraphNode_SetQuestFlag_Execute_QuestNull", "DialogGraphNode_SetQuestFlag:Execute:Quest is null or invalid."));
			return true;
		}
